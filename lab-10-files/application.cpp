...
struct Particle {
    vec3 x; //position of particle
    vec3 v; //velocity of the particle
    vec3 f; //force accumulator
    float m; //# mass of the particle
    vec3 color; //# for a simpler version you can use “vec3 color”

    void Euler_Step(float h) {
        v = v + f*(h/m);
        x = x + v*h;
    } //# update v and x with an Forward Euler Step (see particle.pdf)
    void Reset_Forces() {
        this->f = vec3(0,0,0);
    } //# reset force to 0 vector;
    void Handle_Boundary(double coeff_resititution,double damping) {
        if(x[1] <= 0) {
            x[1] = -x[1];
            if (v[1] < 0) {
    ...
    //# reflect particle on ground and damp according to the document
...
void addParticles(int n) {
    for(int i = 0; i < n; i++) {
        Particle p;
        p.m = 1.0;
        p.x = vec3(frandom(-0.2,0.2), 0.05, frandom(-0.2,0.2));
        p.v = vec3(10*p.x[0], frandom(1,10),10*p.x[2]);
        p.color = vec3(1, frandom(0,1), 0);
...
void application::draw_event()
{
    ...
    if (!paused) {
        ...
        for(unsigned i = 0; i < particles.size(); i++) {
            ...
            particles.at(i).f = vec3(0,-9.8,0);
            particles.at(i).Euler_Step(0.015);
            particles.at(i).Handle_Boundary(0, 1);
            particles.at(i).color *= 0.95;
        }
    ...
    glBegin(GL_LINES);
        for(unsigned int i = 0; i < particles.size(); i++) {
            ...
            glVertex3f(particles.at(i).x[0] + 0.04* particles.at(i).v[0],
                    particles.at(i).x[1]+ 0.04* particles.at(i).v[1],
                    particles.at(i).x[2]+ 0.04* particles.at(i).v[2]);
        }
        //
